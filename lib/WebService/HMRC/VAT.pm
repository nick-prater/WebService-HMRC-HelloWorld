...
WebService::HMRC::VAT - Access the UK HMRC VAT API
...
=head1 SYNOPSIS
    ...
    $vat->auth->access_token->( $my_access_token );
...
=head2 liabilities({ from => 'YYYY-MM-DD', to => 'YYYY-MM-DD' })
...
=head2 payments({ from => 'YYYY-MM-DD', to => 'YYYY-MM-DD' })
...
Note that some fields are rounded to the nearest pound (being the precision
to which those are required to be submitted).
